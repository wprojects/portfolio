...
get_crm_hotspot_records = "select record_id, status, address, product_category, location_level from zoho_crm_hotspot_records"
insert_hotspot_record = "insert into zoho_update_location_records (accessed, record_id, name, address, status, lat, lng, location, location_level, location_hex) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
counter = 0
for (record_id, status, address, product_category, location_level) in hotspot_records_count:
...
db_count = -1
for (record_id, status, address, product_category, location_level) in hotspot_records:
    if status == "Online" or status == "Offline":
        yo = ""
        if 'Radio' in product_category:
            pass
            ...
            lng = f"{lng:.15}"
            location = f"{lat},{lng}"
            location_hex = data['location_hex']
            ...
            if db_count == counter:
                break
            args = (dt.datetime.now(), record_id, name, address, status, lat, lng, location, location_level, location_hex)
