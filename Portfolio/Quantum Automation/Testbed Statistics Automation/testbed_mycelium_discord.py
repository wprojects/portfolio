...
daily_total = 0
# get_daily_total = run_query(get_hotspots_total)
# for (record_id, price) in get_daily_total:
#     daily_total = daily_total + price
# print(f"{daily_total:.2f}")
...
# testing = run_query(group_by)
# for (record_id, status) in testing:
#     print('Hotspot Online: ', len(status))
...
# hotspot_5g_query = run_query(get_crm_5g_hotspot_records)
...
#Mycelium Discord
conn = http.client.HTTPSConnection('eoqc7khuadf9rld.m.pipedream.net')
#WW Discord
# conn = http.client.HTTPSConnection('eodjkr73nf64zbr.m.pipedream.net')
