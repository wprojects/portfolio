...
helium = 'https://api.datacake.co/v1/devices/example/historic_data/?fields=TEMPERATURE,HUMIDITY,PRESSURE,GAS,WIND&resolution=raw&timeframe_start='+str(today)+'T00:00:00Z&timeframe_end='+str(today)+'T23:59:59Z'
ttn  = 'https://api.datacake.co/v1/devices/example/historic_data/?fields=TEMPERATURE,HUMIDITY,PRESSURE,GAS&resolution=raw&timeframe_start='+str(today)+'T10:39:00Z&timeframe_end='+str(today)+'T10:00:00Z'
...
def weather_tweet():
    r = requests.get('https://api.datacake.co/v1/devices/example/historic_data/?fields=TEMPERATURE,HUMIDITY,PRESSURE,GAS,WIND&resolution=raw&timeframe_start='+str(today)+'T00:00:00Z&timeframe_end='+str(today)+'T23:59:59Z', headers={'Authorization': 'Token '})
...
#Soil Sensor Values
    r = requests.get('https://api.datacake.co/v1/devices/example/historic_data/?fields=SOIL_TEMPERATURE,SOIL_MOISTURE&resolution=raw&timeframe_start='+str(today)+'T00:00:00Z&timeframe_end='+str(today)+'T23:59:59Z', headers={'Authorization': 'Token '})
    data = r.json()
    time_format = "%Y-%m-%dT%H:%M:%SZ"
    data = sorted(data, reverse=True, key=lambda x: datetime.strptime(x['time'], time_format))
    print('Data: ', data)
