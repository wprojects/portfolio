...
@bot.event
async def on_ready():
    my_guild = bot.get_guild(787027688548073522)
    ...
    bot_feed = bot.get_channel(1083786228022915102)
...
#Join Channel
VERIFY_CHANNEL = 1105844627417870376
#Test Channel
# VERIFY_CHANNEL = 1083786228022915102
...
async def verify(interaction: discord.Interaction , wallet_address: str):
    ...
    while True:
        response = r.get(f'https://samwins.pythonanywhere.com/check_waitlist?page={page}&items_per_page={items_per_page}')
    ...
    if solana_pattern.match(wallet_address):
        if any(entry['discord_id'] == str(user_id) for entry in waitlist_data):
            ...
            if existing_entry['wallet_address'] != wallet_address:
                ...
                url = "https://samwins.pythonanywhere.com/submit_stickerclub"
                data = {
    ...
    # if user_id in last_address and last_address[user_id] == wallet_address:
        #Check Spot on Wait/White list
        response = r.get('https://samwins.pythonanywhere.com/sticker_whitelist')
        if response.status_code == 200:
        ...
        if index != -1:  # Check if a valid index was found
            ...
            channel_link = f"https://discord.com/channels/{interaction.guild.id}/1098245554284281856"
    ...
    if solana_pattern.match(wallet_address):
        ...
        await interaction.response.defer(ephemeral=True)
        await interaction.followup.send(embed=embed, ephemeral=True)
        # args = (wallet_address, dt.datetime.now(), user_id, username)
...
async def sticker_stats(guild, bot):
    test_channel = bot.get_channel(1083786228022915102) #Test Channel
    sticker_club_channel = bot.get_channel(1098245554284281856) #Sticker Club Channel
    page = 1
    items_per_page = 100
    waitlist_count = 0
    while True:
        response = r.get(f'https://samwins.pythonanywhere.com/sticker_waitlist?page={page}&items_per_page={items_per_page}')
        if response.status_code == 200:
            json_data = response.json()
            if len(json_data) == 0:
                break
...
async def my_task(guild, bot):
    channel = bot.get_channel(1098245554284281856)
    sticker_club_channel = bot.get_channel(1098245554284281856) #Sticker Club Channel
    # sticker_club_channel = bot.get_channel(1083786228022915102) #Testing bots channel
    response = r.get('https://samwins.pythonanywhere.com/sticker_whitelist')
    if response.status_code == 200:
        json_data = response.json()
        mentions = []
    ...
    response_welcome = r.get('https://samwins.pythonanywhere.com/sticker_whitelist_welcome')
...
# @bot.tree.command(name="stickerclub", description="Enter your SOL address to join the waitlist.")
# async def stickerclub(interaction: discord.Interaction, wallet_address: str):
#     user_id = interaction.user.id
#     page = 1
#     items_per_page = 100
#     waitlist_data = []
...
#         if response.status_code == 200:
#             json_data = response.json()
#             json_data.reverse()  # This reverses the order so the most recent is last
...
#             channel_link = f"https://discord.com/channels/{interaction.guild.id}/1098245554284281856"
#             # await interaction.response.send_message("You're already on the waitlist. In the meantime, come hang out in **#wait-list.**", ephemeral=True)
#             embed = discord.Embed(title=f"Sticker Club", description="You're already in the club!", color=0xFFD700)
#             embed.add_field(name=f"You're currently #{index + 1} on the drop list!", value=channel_link, inline=False)
#             # await interaction.response.defer(embed=embed, ephemeral=True)
#             await interaction.response.defer(ephemeral=True)
#             # Perform time-consuming operations here
...
#             channel_link = f"https://discord.com/channels/{interaction.guild.id}/1098245458159206540"
#             # await interaction.response.send_message("You're already on the waitlist. In the meantime, come hang out in **#wait-list.**", ephemeral=True)
#             embed = discord.Embed(title=f"Sticker Club Waitlist", description="You're already on the waitlist.", color=0xFFD700)
#             embed.add_field(name=f"You're already on the waitlist. In the meantime, come hang out in **#wait-list.**", value=channel_link, inline=False)
#             # await interaction.response.defer(embed=embed, ephemeral=True)
#             await interaction.response.defer(ephemeral=True)
#             # Perform time-consuming operations here
...
#         await interaction.response.defer(ephemeral=True)
#         await interaction.followup.send("Please try again with a valid SOL address.", ephemeral=True)
#         # await interaction.response.send_message("Please try again with a valid SOL address.", ephemeral=True)
